 ...
 *
 * ✅ 2026-09-09 — 발신번호 인증 완료. SENDER_NUMBER 를 01053226995 로 바꿨습니다.
 *    이제 업체 폰에 「백석」이 아니라 원당 번호로 뜹니다.
 *
 * ⚠️ 아직 백석 번호(01041216995)로 남아 있는 것들이 있습니다.
 *       VENDOR_NUMBER          고기 발주를 받는 업체
 *       WONDANG_ADMIN          「발주 넣으세요」 알림을 받는 사람
 *       FOOD.PHONES 여러 곳     미락·콩나물·주류·음료수 등
 *    이건 「받는 쪽」 번호라 사장님 번호로 바꾸면 안 됩니다.
...
const CONFIG = {
  ...
  // ✅ 2026-09-09 — 솔라피 발신번호 인증 완료. 원당 관리자 번호로 바꿨습니다.
  ...
  // ── [v2.0 추가] 식자재 발주 설정 ─────────────────────────
  FOOD: {
    // ⚠️ 2026-09-20 — 지점별로 탭을 나눴습니다 (사장님 요청).
    //    그전엔 백석·원당이 한 탭에 섞여 쌓여서 나중에 보기가 번거로웠습니다.
    //    ⚠️ 백석은 접미사 없이 그대로 둡니다. 기존 기록을 지키기 위해서입니다.
    //    ⚠️ 탭이 없으면 코드가 헤더까지 알아서 만듭니다. 손으로 만들 필요 없습니다.
    SHEET_NAME     : '식자재발주_원당',
    SMS_MAX_BYTES  : 90,
    LMS_MAX_BYTES  : 2000,
    // 업체 전화번호 (숫자만, 하이픈 없이) — 확인 후 채워넣기
    // ⚠️ 2026-09-12 — 화면(food.html)이 보내는 번호를 먼저 씁니다.
    //    여기는 화면이 번호를 안 보냈을 때의 예비값입니다. 둘을 같이 맞춰둡니다.
    PHONES: {
      '미락'    : '01089421859',
      '콩나물'  : '01062333466',
      '주류'    : '01056219640',   // 2026-09-19 원당 주류업체 (백석 01056079640 과 다름)
      '음료수'  : '01037982411',
      '백석'    : '01041216995',   // 원당은 백석에서 대파김치를 받아옵니다
      '원당'    : '01053226995',
      '네이버'  : '01053226995',
      '배달관련': '01053226995',
      '사장님'  : '01053226995',
  ...
  // ⚠️ 2026-09-20 — 지점별 탭 분리. 백석은 접미사 없음(기존 기록 유지).
  SHEET_ORDER     : '발주기록_원당',
  SHEET_STOCK     : '입고기록_원당',
  SHEET_FAIL      : '발송실패_원당',   // 실패 이력이 쌓이는 곳
};
